...
q = 1
function fun_u(x)
    return 2^(4*p) * x[1]^p*(1-q*x[1])^p*x[2]^p*(1-q*x[2])^p
end

function fun_rhs(x)
    ans = -2^(4*p) * (
    (p*(p-1)* x[1]^(p-2)*(1-q*x[1])^p + q^2*p*(p-1)*x[1]^p*(1-q*x[1])^(p-2)-2*p^2*q*x[1]^(p-1)*(1-q*x[1])^(p-1)) *  x[2]^p*(1-q*x[2])^p
    + (p*(p-1)* x[2]^(p-2)*(1-q*x[2])^p + q^2*p*(p-1)*x[2]^p*(1-q*x[2])^(p-2)-2*p^2*q*x[2]^(p-1)*(1-q*x[2])^(p-1)) *  x[1]^p*(1-q*x[1])^p
    )
    return ans + α*fun_u(x)^m 
end
